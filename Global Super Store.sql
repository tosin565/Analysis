...
--Monthly Trend
SELECT DATENAME(MONTH, order_date) Month_name, COUNT(DISTINCT Order_id) Total_order
FROM [Global Super Store Merge]
...
RankedSales AS (
    SELECT 
        Country, 
        Total_sales,
        ROW_NUMBER() OVER (ORDER BY Total_sales DESC) AS TopRank,
        ROW_NUMBER() OVER (ORDER BY Total_sales ASC) AS BottomRank
    FROM 
        Countrysales
)
SELECT 
